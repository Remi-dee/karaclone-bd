...
import { TransactionFee, TransactionFeeDocument } from './schema/transaction-fee.schema';
import { Model, ObjectId } from 'mongoose';
import { CreateTransactionFeeDTO, UpdateTransactionFeeDTO } from './transaction-fee.dto';
...
async findAll(): Promise<TransactionFee[]> {
    ...
    async createTransactionFee(
        ...
        const createdTransactionFee = await this.transactionFeeModel.create(transactionFeeData);
      ...
      async deleteTransactionFee(userId: ObjectId, feeId: ObjectId): Promise<void> {
        // Check if the user is an admin
        await this.checkAdminPermission(userId);
    
        // Find the transaction fee by ID and delete it
        const result = await this.transactionFeeModel.deleteOne({ _id: feeId }).exec();
      ...
      private async checkAdminPermission(userId: ObjectId): Promise<void> {
        ...
        if (!user || user.account_type !== 'admin') {
          throw new Error('User not found or not authorized to manage transaction fees');
        }
      }
